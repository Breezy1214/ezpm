...
mod tests {
    ...
    #[test]
    fn discovery_retains_multiple_projects_for_explicit_selection() {
        let dir = TempDir::new().unwrap();
        write_project(dir.path(), "lobby.project.json", r#"{"tree":{}}"#);
        write_project(dir.path(), "game.project.json", r#"{"tree":{}}"#);

        let discovery = discover_rojo_projects(dir.path()).unwrap();
        assert_eq!(discovery.projects.len(), 2);
        assert_eq!(
            discovery.projects[0].path.file_name().unwrap(),
            "game.project.json"
        );
    }
